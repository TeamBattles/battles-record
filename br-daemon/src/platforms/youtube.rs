...
pub fn is_bun_available() -> bool {
    if let Some(path) = crate::libraries::platform::resolve_binary("bun", None) {
...
pub fn is_ytdlp_available() -> bool {
    if let Some(path) = crate::libraries::platform::resolve_binary("yt-dlp", None) {
        std::process::Command::new(&path)
            .arg("--version")
            .stdout(Stdio::null())
            .stderr(Stdio::null())
...
impl YoutubePlatform {
    ...
    async fn ytdlp_json(&self, url: &str) -> Result<YtdlpVideoInfo, YtdlpError> {
        let ytdlp_path = crate::libraries::platform::resolve_binary("yt-dlp", None)
            .ok_or(YtdlpError::YtdlpNotFound)?;
