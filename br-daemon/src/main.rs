...
use br_daemon::downloads::events::DownloadEvent;
...
async fn main() -> anyhow::Result<()> {
    ...
    // Allow --downloads-dir flag to override downloads directory
    if let Some(downloads_dir) = std::env::args().skip_while(|a| a != "--downloads-dir").nth(1) {
        tracing::info!(
            "Downloads directory override via --downloads-dir flag: {}",
            downloads_dir
        );
        config.downloads.directory = PathBuf::from(downloads_dir);
    }
    ...
    tokio::spawn(async move {
        if channel_count > 0 {
            tracing::info!(
                "Running initial channel check ({} channels)...",
                channel_count
            );
    ...
    // Create library manager for external binary management
    let library_manager = Arc::new(tokio::sync::Mutex::new(
        br_daemon::libraries::LibraryManager::new(
            config.read().libraries.clone(),
            config.read().post_processing.ffmpeg_path.clone(),
        ),
    ));

    // Create download manager for VOD downloads
    let (downloads_config, downloads_dir) = {
        let cfg = config.read();
        (
            cfg.downloads.clone(),
            std::path::PathBuf::from(&cfg.downloads.directory),
        )
    };
    let download_manager = Arc::new(
        br_daemon::downloads::DownloadManager::new(
            downloads_config,
            downloads_dir,
    ...
    // Forward download events to the main event bus
    {
        let mut download_rx = download_manager.subscribe();
        let event_tx = event_tx.clone();
        tokio::spawn(async move {
            loop {
                match download_rx.recv().await {
                    Ok(event) => {
                        let manager_event = convert_download_event(event);
                        let _ = event_tx.send(manager_event);
                    }
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        tracing::warn!("Download event receiver lagged by {} events", n);
                    }
    ...
    // Spawn download cleanup worker
    {
        let cleanup_download_manager = download_manager.clone();
        let cleanup_config = config.clone();
        let interval_hours = config.read().downloads.retention.cleanup_interval_hours;
        if interval_hours > 0 {
            tokio::spawn(async move {
                ...
                loop {
                    interval.tick().await;
                    let retention_days = cleanup_config.read().downloads.retention.max_age_days;
                    match cleanup_download_manager.run_cleanup(retention_days).await {
                        Ok(count) => {
                            if count > 0 {
                                tracing::info!(deleted = count, "Download cleanup completed");
                            } else {
                                tracing::debug!("Download cleanup: nothing to delete");
                            }
                        }
                        Err(e) => {
                            tracing::error!(error = ?e, "Download cleanup failed");
                        }
                    }
                }
            });
            tracing::info!(
                "Download cleanup worker started (interval: {} hours)",
                interval_hours
            );
        } else {
            tracing::info!("Download cleanup disabled (cleanup_interval_hours = 0)");
        }
    }
    ...
    // Create pairing manager for extension authentication
    let data_dir = config.read().storage.recordings_dir.clone();
    let pairing_manager = Arc::new(tokio::sync::RwLock::new(
        br_daemon::extension::pairing::PairingManager::new(&data_dir)
            .context("Failed to initialize pairing manager")?,
    ));
    let extension_connections: Arc<
        tokio::sync::RwLock<
            std::collections::HashMap<uuid::Uuid, br_daemon::extension::connection::ConnectionInfo>,
        >,
    > = Arc::new(tokio::sync::RwLock::new(std::collections::HashMap::new()));
    let extension_message_senders: Arc<
        tokio::sync::RwLock<
            std::collections::HashMap<uuid::Uuid, tokio::sync::mpsc::Sender<br_daemon::extension::protocol::DaemonMessage>>,
        >,
    > = Arc::new(tokio::sync::RwLock::new(std::collections::HashMap::new()));

    // Start extension server if enabled (before AppState so we get the port)
    let (extension_port, extension_shutdown_tx) = if config.read().extension.enabled {
        let ext_config = config.read().extension.clone();
        match br_daemon::extension::ExtensionServer::start(
            ext_config,
            pairing_manager.clone(),
            extension_connections.clone(),
            extension_message_senders.clone(),
            library_manager.clone(),
        ...
        .await
        {
            Ok((port, _handle, shutdown_tx)) => {
                tracing::info!("Extension WebSocket server listening on port {}", port);
                (Some(port), Some(shutdown_tx))
            }
            Err(e) => {
                tracing::error!(error = ?e, "Failed to start extension server");
                (None, None)
            }
        }
    } else {
        tracing::info!("Extension server disabled");
        (None, None)
    };
    ...
    let state = Arc::new(AppState {
        ...
        extension_message_senders,
        extension_shutdown_tx: extension_shutdown_tx.clone(),
        extension_port,
    });
    ...
    // Shutdown extension server
    if let Some(tx) = extension_shutdown_tx {
        if tx.send(()).await.is_err() {
            tracing::debug!("Extension server shutdown channel already closed");
        }
...
fn convert_download_event(event: DownloadEvent) -> ManagerEvent {
    match event {
        DownloadEvent::Queued { job } => ManagerEvent::DownloadQueued { job },
        ...
        DownloadEvent::Paused { download_id } => ManagerEvent::DownloadPaused { download_id },
        DownloadEvent::Resumed { download_id } => ManagerEvent::DownloadResumed { download_id },
        DownloadEvent::Cancelled { download_id } => {
            ManagerEvent::DownloadCancelled { download_id }
        }
    }
}
