...
impl NotificationPayload {
    ...
    /** Create a notification for a completed download. */
    pub fn download_complete(channel: &str, filepath: &str, filesize: u64) -> Self {
        let size_mb = filesize as f64 / (1024.0 * 1024.0);
        Self {
            event_type: NotificationType::DownloadComplete,
            timestamp: Utc::now(),
            channel_name: Some(channel.to_string()),
            platform: None,
            title: None,
            message: format!(
                "Download complete for {} ({:.2} MB)",
                channel, size_mb
            ),
            data: Some(serde_json::json!({
                "filepath": filepath,
    ...
    /** Create a notification for a failed download. */
    pub fn download_failed(channel: &str, error: &str) -> Self {
        Self {
            event_type: NotificationType::DownloadFailed,
            timestamp: Utc::now(),
            channel_name: Some(channel.to_string()),
            platform: None,
            title: None,
            message: format!("Download failed for {}: {}", channel, error),
            data: None,
        }
    }
