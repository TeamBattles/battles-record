...
fn get_emoji(event_type: NotificationType) -> &'static str {
    match event_type {
        ...
        NotificationType::DownloadComplete => "\u{2B07}\u{FE0F}", // Down arrow
        NotificationType::DownloadFailed => "\u{26A0}\u{FE0F}", // Warning
...
fn get_color(event_type: NotificationType) -> u32 {
    match event_type {
        ...
        | NotificationType::DownloadComplete => colors::INFO,
        NotificationType::DownloadFailed | NotificationType::Error => colors::ERROR,
    }
}
...
fn build_title(payload: &NotificationPayload) -> String {
    ...
    match payload.event_type {
        ...
        NotificationType::DownloadComplete => {
            format!("{} Download complete for {}", emoji, channel_display)
        }
        NotificationType::DownloadFailed => {
            format!("{} Download failed for {}", emoji, channel_display)
        }
