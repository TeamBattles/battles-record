...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>, username: String) {
    ...
    // Send initial state
    let initial_state = build_initial_state(&state).await;
...
async fn build_initial_state(state: &AppState) -> WsEvent {
    ...
    let active_downloads = state.download_manager.get_queue().await;
...
fn convert_manager_event(event: ManagerEvent) -> Option<WsEvent> {
    match event {
        ...
        ManagerEvent::DownloadQueued { job } => Some(WsEvent::DownloadQueued {
            download_id: job.id,
            url: job.url,
            title: job.title,
        ...
        ManagerEvent::DownloadFailed {
            ...
            error,
            update_available,
        } => Some(WsEvent::DownloadFailed {
            download_id,
            channel_name,
            error,
            update_available,
        }),
        ManagerEvent::DownloadPaused { download_id } => {
            Some(WsEvent::DownloadPaused { download_id })
        }
        ManagerEvent::DownloadResumed { download_id } => {
            Some(WsEvent::DownloadResumed { download_id })
        }
        ManagerEvent::DownloadCancelled { download_id } => {
            Some(WsEvent::DownloadCancelled { download_id })
        }
        ManagerEvent::LibraryStatusChanged {
            library,
            installed,
            version,
        } => Some(WsEvent::LibraryStatusChanged {
            library,
            installed,
            version,
        }),
        ManagerEvent::ExtensionConnected {
            client_id,
            identifier,
        } => Some(WsEvent::ExtensionConnected {
            client_id,
            identifier,
        }),
        ManagerEvent::ExtensionDisconnected {
            client_id,
            identifier,
        } => Some(WsEvent::ExtensionDisconnected {
            client_id,
            identifier,
        }),
        ManagerEvent::ChannelAdded {
            ...
            channel: Channel {
                ...
                quota_gb: None,
                retention_days: None,
                quota_status: crate::types::QuotaStatus::Ok,
                quota_used_bytes: 0,
                quota_percent: 0,
