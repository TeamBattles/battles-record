...
pub async fn start_local_daemon(
    ...
    let (config_path, resolved_data_dir, resolved_library_dir, resolved_downloads_dir) = {
        let daemon = state.lock().map_err(|e| e.to_string())?;
        ...
        // Prefer passed downloads_dir parameter, then state's downloads_dir, then default (sibling of data_dir)
        let downloads = downloads_dir.map(PathBuf::from)
            .or_else(|| daemon.downloads_dir.clone())
            .unwrap_or_else(|| data.parent().unwrap_or(&data).join("downloads"));
        (config, data, library, downloads)
    };
    ...
    log::info!("Downloads directory: {:?}", resolved_downloads_dir);
    ...
    // Update status to starting
    {
        ...
        daemon.downloads_dir = Some(resolved_downloads_dir.clone());
    }
    ...
    // Add downloads-dir (always passed, like --data-dir)
    args.push("--downloads-dir".to_string());
    args.push(resolved_downloads_dir.to_string_lossy().to_string());
...
pub async fn restart_local_daemon(
    ...
    start_local_daemon(app, state, None, None, None).await
}
...
pub fn get_local_daemon_paths(
    ...
    if let Ok(daemon) = state.lock() {
        if daemon.config_path.is_some() || daemon.data_dir.is_some() {
            // If downloads_dir not explicitly set, the daemon uses its TOML config default.
            // We don't know the resolved path here, so leave it to the REST API to provide.
            return Ok(LocalDaemonPaths {
                ...
                downloads_dir: daemon.downloads_dir.as_ref().map(|p| p.to_string_lossy().to_string()),
            });
        }
    }
    ...
    let default_data_dir = app_data_dir.join("recordings");
    let default_downloads_dir = app_data_dir.join("downloads");
    Ok(LocalDaemonPaths {
        config_path: Some(app_data_dir.join("br-config.toml").to_string_lossy().to_string()),
        data_dir: Some(default_data_dir.to_string_lossy().to_string()),
        library_dir: None,
        downloads_dir: Some(default_downloads_dir.to_string_lossy().to_string()),
    })
}
...
pub async fn set_local_daemon_paths(
    ...
    if let Some(ref path) = downloads_dir {
        validate_path(path)?;
    }
    ...
    // Validate downloads_dir exists or create it
    if let Some(ref dir) = downloads_dir {
        let path = std::path::Path::new(dir);
        if !path.exists() {
            tokio::fs::create_dir_all(path)
                .await
    ...
    // Update daemon state with new paths
    {
        ...
        if let Some(dir) = downloads_dir {
            daemon.downloads_dir = Some(PathBuf::from(dir));
        }
    }
