...
import type { DownloadSummary, Platform } from './types';
...
export type WebSocketEvent =
	...
	| { type: 'download_paused'; download_id: string }
	| { type: 'download_resumed'; download_id: string }
	| { type: 'download_cancelled'; download_id: string }
	| { type: 'channel_added'; channel: BackendChannel }
	| { type: 'channel_removed'; channel_id: string; name: string; platform: string }
	| { type: 'extension_connected'; client_id: string; identifier: string }
	| { type: 'extension_disconnected'; client_id: string; identifier: string }
	| { type: 'library_status_changed'; library: string; installed: boolean; version?: string }
	| {
			type: 'connected';
			channels: BackendChannel[];
			active_recordings: unknown[];
			active_downloads: DownloadSummary[];
	  }
