	...
	private handleEvent(event: WebSocketEvent) {
		...
		switch (event.type) {
			...
			case 'quota_status_changed':
				...
				break;
			case 'channel_added': {
				const newChannel = transformChannel(event.channel);
				// Avoid duplicates
			...
			case 'channel_removed':
				this.channels = this.channels.filter((c) => c.id !== event.channel_id);
				if (this.selectedChannelId === event.channel_id) {
					this.selectedChannelId = null;
				}
				this.profileCache.delete(event.channel_id);
				this.profileLoading.delete(event.channel_id);
				break;
		}
	}
