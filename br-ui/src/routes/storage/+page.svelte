	...
	{:else if storageStore.stats}
		...
		{:else}
			...
			<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<StatusCard
					label="Downloads"
					value="{storageStore.totalDownloadsSizeGB} GB"
					icon={Download}
					status="default"
				/>
		...
		{#if storageStore.sortedDownloadChannelStats.length > 0}
			<div class="relative border border-border bg-card overflow-hidden">
				<CornerBrackets class="z-10" />
				<div class="flex items-center gap-2 border-b border-border/60 bg-muted/30 px-4 py-2">
					...
					<span class="rounded bg-zinc-200 px-1.5 py-0.5 font-mono text-[10px] text-zinc-600 dark:bg-zinc-800 dark:text-zinc-500">
						{storageStore.downloadStats?.total_downloads ?? 0}
					</span>
				</div>
				{#if breakpointStore.isMobile}
					<div class="divide-y divide-border/30">
						{#each storageStore.sortedDownloadChannelStats as channelStats}
							<div class="p-4 space-y-2">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-2">
										<PlatformIcon platform={channelStats.platform as 'twitch' | 'youtube' | 'kick'} class="w-4 h-4 text-zinc-500" />
										<span class="font-mono text-sm">{channelStats.channel}</span>
									</div>
									<span class="font-mono text-xs text-zinc-400">{storageStore.formatBytes(channelStats.size_bytes)}</span>
								</div>
								<StorageProgressBar percent={storageStore.getDownloadChannelPercent(channelStats)} size="sm" />
								<span class="font-mono text-[10px] text-zinc-500">{channelStats.count} file{channelStats.count !== 1 ? 's' : ''}</span>
							</div>
						{/each}
					</div>
				{:else}
					<table class="w-full">
						<thead>
							<tr class="border-b border-border/60">
								...
								<th class="px-4 py-2 text-left">
									<span class="font-mono text-[10px] uppercase tracking-wider text-zinc-500">Channel</span>
								</th>
								<th class="px-4 py-2 text-left">
									<span class="font-mono text-[10px] uppercase tracking-wider text-zinc-500">Usage</span>
								</th>
								<th class="px-4 py-2 text-left">
									<span class="font-mono text-[10px] uppercase tracking-wider text-zinc-500">Count</span>
								</th>
								<th class="px-4 py-2 text-left">
									<span class="font-mono text-[10px] uppercase tracking-wider text-zinc-500">Size</span>
								</th>
							</tr>
						</thead>
						<tbody>
							{#each storageStore.sortedDownloadChannelStats as channelStats}
								{@const percent = storageStore.getDownloadChannelPercent(channelStats)}
								<tr class="border-b border-border/30 hover:bg-muted/30 transition-colors">
									<td class="px-4 py-3">
										<PlatformIcon platform={channelStats.platform as 'twitch' | 'youtube' | 'kick'} class="w-4 h-4 text-zinc-500" />
									</td>
									...
									<td class="px-4 py-3">
										<div class="flex items-center gap-2">
											<div class="flex-1"><StorageProgressBar {percent} size="sm" /></div>
											<span class="font-mono text-[10px] text-zinc-500 w-8 text-right">{percent}%</span>
										</div>
									</td>
									<td class="px-4 py-3">
										<span class="font-mono text-xs text-zinc-400">{channelStats.count}</span>
		...
		<div class="relative border border-border bg-card overflow-hidden">
			...
			<div class="divide-y divide-border/30">
				...
				<div class="px-4 py-3">
					...
					<p class="font-mono text-xs text-zinc-300 break-all">
						{storageStore.downloadStats?.downloads_dir ?? 'Not configured'}
					</p>
					<p class="font-mono text-[10px] text-zinc-500 mt-1">
						Files downloaded via browser extension
					</p>
				</div>
				<div class="px-4 py-3">
					<p class="font-mono text-[10px] uppercase tracking-wider text-zinc-500 mb-1">
